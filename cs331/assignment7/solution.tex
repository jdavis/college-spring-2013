...
\begin{homeworkProblem}
    ...
    \begin{enumerate}
        \item \(\{
                \left[\frac{ab}{a}\right],
                \left[\frac{bb}{ab}\right],
                \left[\frac{aa}{ba}\right],
                \left[\frac{cc}{bc}\right],
                \left[\frac{aa}{ca}\right],
                \left[\frac{d}{cd}\right]
            \}\)
        \item \(\{
                \left[\frac{ab}{a}\right],
                \left[\frac{bb}{ab}\right],
                \left[\frac{aa}{ba}\right],
                \left[\frac{c}{bc}\right],
                \left[\frac{aa}{ca}\right],
                \left[\frac{d}{cd}\right]
            \}\)
    \end{enumerate}
    ...
    This is a solution because if we read across the top, we get \(abaabbccaabbccd\) and
    if we read across the bottom we get \(abaabbccaabbccd\). Thus we have solved this
    given PCB problem.
    \\
    ...
    This part is almost identical to the first part, except there is one
    missing \(c\) on the 4th tile. In using the tiles from the bottom and not
    including the 4th tile, there will be two \(c\)'s. Taking the 4th tile into
    account, this is the only way to add a \(c\) to the top, except it also
    adds a \(c\) to the bottom. Thus if \(n_1\) is the number of \(c\)s on the top
    and \(n_2\) is the number of \(c\)'s on the bottom, then \(n_1 + 2 \leq n_2\).
    Thus we will never be able to balance the number of \(c\)'s.
\end{homeworkProblem}
...
\begin{homeworkProblem}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \item Run \(HALT_{TM}\) with input \(w\), if it accepts, accept, else reject.
        \end{enumerate}
