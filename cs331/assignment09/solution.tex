...
\newcommand{\hmwkDueDate}{April 18, 2013 at 11:59pm}
...
\begin{homeworkProblem}
    ...
    \begin{proof}
        ...
        Let this function the TM \(N\) and constructed as follows:
        \\

        \(N = \)`` On input string \(w\):
        \begin{enumerate}
            \item Run \(M_2\) on \(w\)
            \item If \(M_2\) accepts, then output what \(M_2\) outputs and \(accept\).
            \item If \(M_2\) rejects, then \(reject\).''
        \end{enumerate}
        ...
        we can't map it to a value that isn't in \(\Sigma^*\). Thus any langauge in P
        besides \(\emptyset\) and \(\Sigma^*\) can be polynomial reduced to
        another language in \(P\).
    \end{proof}
\end{homeworkProblem}
